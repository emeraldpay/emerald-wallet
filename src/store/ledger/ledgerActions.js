...
function loadInfo(hdpath: string, addr: string) {
  return (dispatch, getState, api) => {
    return api.geth.eth.getBalance(addr).then((result) => {
      dispatch({
      ...
      return api.geth.eth.getTransactionCount(addr).then((result) => {
        dispatch({
  ...
  return (dispatch, getState) => {
    ...
    connectionStart = () => {
      ...
      if (watchEnabled) {
        ...
        return isWatchingConnection = setTimeout(connectionStart, 3000);
      }
    };
