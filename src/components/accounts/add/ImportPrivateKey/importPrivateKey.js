...
class ImportPrivateKey extends React.Component {
  ...
  submit() {
    const { handleSubmit, importAccounts, dispatch } = this.props;
    ...
    handleSubmit().then((result) => {
      if (result.error) {
        ...
        const p = importAccounts.findKey((acc) => acc.get('id') === this.state.accountId);
        const account = p && (p >= 0) && importAccounts.get(p);
