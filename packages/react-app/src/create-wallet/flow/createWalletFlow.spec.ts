...
describe('Select Source Type', () => {
  it('when seed selected', () => {
    ...
    const act = start.applySource({ type: KeySourceType.SEED_SELECTED, id: 'test' });

    expect(act.getCurrentStep().code).toBe(STEP_CODE.OPTIONS);
    expect(act.getCurrentStep().index).toBe(1);

    expect(act.getSteps().length).toBe(7);
    ...
    expect(act.getSteps()[5].code).toBe(STEP_CODE.CREATING);
    expect(act.getSteps()[6].code).toBe(STEP_CODE.CREATED);
  });
});
...
describe('Select Options', () => {
  it('empty label', () => {
    ...
    const act = start.applyOptions({ label: '' }).applyNext();
  ...
  it('some label', () => {
    const start = new CreateWalletFlow(noCreate).applySource({ type: KeySourceType.SEED_SELECTED, id: 'test' });
    const act = start.applyOptions({ label: 'some label' }).applyNext();
  ...
  it('goes to import key', () => {
    ...
    const start = CreateWalletFlow.create(initial, STEP_CODE.OPTIONS, noCreate);
    const act = start.applyOptions({ label: 'test' }).applyNext();
...
describe('Generate Mnemonic', () => {
  it('goes to lock', () => {
    ...
    const act = start.applyMnemonic('test test test', undefined);
...
describe('Import Mnemonic', () => {
  it('goes to lock', () => {
    ...
    const act = start.applyMnemonic('test test test', 'test');
...
describe('Lock seed', () => {
  it('goes to blockchains', () => {
    ...
    const act = start.applyMnemonicSaved('test-uuid', 'testpwd');
...
describe('Select Blockchain', () => {
  it('goes to unlock', () => {
    ...
    let act = start.applyBlockchains([BlockchainCode.ETH]);
  ...
  it('create if import pk', () => {
    ...
    const start = CreateWalletFlow.create(initial, STEP_CODE.SELECT_BLOCKCHAIN, (value) => (result = value));
...
describe('Unlock Seed', () => {
  it('unlocked with password', () => {
    ...
    const act = start.applySeedPassword('testpwd');
...
describe('Account', () => {
  it('creates after selection', () => {
    ...
    const start = CreateWalletFlow.create(initial, STEP_CODE.SELECT_HD_ACCOUNT, (value) => (result = value));
