...
const convertTx: EntryHandler<BitcoinEntry> =
  ...
  () => {
    ...
    if (tx != null && workflow.isBitcoinPlainTx(tx)) {
      const createTx = workflow.CreateBitcoinSpeedUpTx.fromPlain(
        {
...
export const restoreBitcoinSpeedUpTx: EntryHandler<BitcoinEntry, Promise<void>> = (data, storeProvider) => async () => {
